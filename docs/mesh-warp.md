# Topology-preserving control-lattice mesh warp

This document describes the vector deformation system used by the
`mesh-warp` fiddle. The reusable implementation lives in
`cc-engine/graphics/mesh_warper.{h,cc}`; artwork generation, panel layout,
...
dimension. The same cap is installed in `MeshWarper`, so both the visible
...
The current fiddle uses `4 × device_scale` pixels on a full-size canvas.
Responsive canvases multiply the logical gap by a factor down to `0.58`, so a
mobile lattice reaches a `2.32` logical-pixel gap. The physical value still
includes device scale, making density independent of device pixel ratio. A
small safety floor and a 60,000-vertex fitting pass prevent pathological
lattices and stay below the mesh's 16-bit index limit. The topology threshold is
one thousandth of its earlier value (`10⁻⁶` instead of `10⁻³`), allowing later
gestures to compress cells much further before selective topology clamping
intervenes.
...
`MeshWarpFiddleOptions`, rather than scattered as anonymous constants. The
...
pointer_step_ratio                 0.015 of shorter dimension/frame
...
The fiddle copies these settings into `MeshWarperOptions` and
...
`HatchLineRect` occupies four of the fourteen shape-selection slots, compared
with one slot for each ordinary geometry family, so hatch patterns appear
substantially more often. Artwork rows and columns are selected from logical
viewport size rather than physical pixels: compact canvases target smaller
cells and fewer columns, while larger canvases expand to as many as seven.
Jitter and radius are cell-relative, keeping the composition legible across
both layouts.

The synthetic pointer also uses relative units. Each rendered frame advances
by 1.5% of the source artwork's shorter dimension, so a high-DPI or mobile
canvas no longer changes the apparent gesture speed. The reusable simulator
performs that conversion through `FakeMouseActions::AdvanceByRatio`; its
absolute-distance `Advance` API remains available for deterministic low-level
tests and non-responsive callers.
