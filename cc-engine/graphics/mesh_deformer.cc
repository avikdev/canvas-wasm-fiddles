#include "graphics/mesh_deformer.h"
...
bool MeshDeformer::Build(const SkRect &bounds,
                         const MeshDeformerOptions &options) {
...
bool MeshDeformer::BeginDrag(const SkPoint &position, float radius) {
...
bool MeshDeformer::UpdateDrag(const SkPoint &position, float intensity) {
...
SkPoint MeshDeformer::MapPoint(const SkPoint &point) const {
...
SkPath MeshDeformer::DeformPath(const SkPath &path) const {
...
SkPath MeshDeformer::ControlGridPath() const {
  if (!IsBuilt()) {
    return {};
  }
...
void MeshDeformer::Draw(SkCanvas *canvas, SkImage *image) const {
...
void MeshDeformer::AddTriangle(std::uint16_t a, std::uint16_t b,
                               std::uint16_t c) {
...
bool MeshDeformer::PreservesTopology(
...
bool MeshDeformer::PreservesVertexTopology(
...
void MeshDeformer::RefreshPositions() {
  for (std::size_t index = 0; index < positions_.size(); ++index) {
    positions_[index] =
