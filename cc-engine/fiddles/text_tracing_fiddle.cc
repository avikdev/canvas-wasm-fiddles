...
bool TextTracingFiddle::EnsureResources() {
  ...
  if (font_manager == nullptr || font_manager->countFamilies() == 0) {
    std::cerr << "[cc-engine/stderr] Text Tracing could not access the shared "
                 "font manager."
              << std::endl;
    return false;
  }
  ...
  if (typeface_ == nullptr) {
    std::cerr << "[cc-engine/stderr] Text Tracing could not resolve the "
                 "default typeface."
              << std::endl;
    return false;
  }
...
bool TextTracingFiddle::RebuildAtomicSegments() {
...
void TextTracingFiddle::Render(double time_seconds) {
  if (!EnsureResources()) {
    return;
  }
  ...
  if (!present.success) {
    std::cerr << "[cc-engine/stderr] Text Tracing could not submit its WebGL "
                 "frame."
              << std::endl;
  }
}

bool TextTracingFiddle::UpdateState(double time_seconds, int width,
                                    int height) {
...
void TextTracingFiddle::DrawFrame(SkCanvas *canvas, int width, int height) {
