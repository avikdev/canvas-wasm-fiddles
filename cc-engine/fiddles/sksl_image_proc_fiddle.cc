...
bool SkslImageProcFiddle::EnsureResources() {
  ...
  if (image_ == nullptr) {
    std::cerr << "[cc-engine/stderr] SkSL image processing requires worker "
                 "image id \""
              << kImgUrl << "\"." << std::endl;
    return false;
  }
  ...
  if (label_typeface_ == nullptr) {
    std::cerr << "[cc-engine/stderr] SkSL image processing could not resolve "
                 "its label typeface."
              << std::endl;
    return false;
  }
  ...
  std::cout << "[cc-engine/stdout] SkSL image processing resources ready: "
               "five non-identity RGB permutations."
            << std::endl;
  return true;
}

bool SkslImageProcFiddle::BuildChannelShaders() {
...
void SkslImageProcFiddle::Render(double time_seconds) {
  if (!EnsureResources()) {
    return;
  }
