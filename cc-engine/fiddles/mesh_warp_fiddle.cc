...
struct MeshLayout {
  float responsive_scale = 1.0F;
  float padding = 0.0F;
  float gap = 0.0F;
  float inset = 0.0F;
  float legend_band = 0.0F;
  float legend_width = 0.0F;
  float legend_height = 0.0F;
  float border_width = 0.0F;
  float pointer_stroke_width = 0.0F;
  float pointer_outline_width = 0.0F;
  float arrow_head_length = 0.0F;
  float grid_stroke_width = 0.0F;
};

MeshLayout MakeMeshLayout(int width, int height, float device_scale) {
  ...
  const float responsive_scale = std::clamp(
      std::min(logical_width / 720.0F, logical_height / 600.0F), 0.58F, 1.0F);
  const float ui_scale = safe_scale * responsive_scale;
  return {
      .responsive_scale = responsive_scale,
      .padding =
          std::max(5.0F * safe_scale, kLogicalOuterPadding * responsive_scale *
                                          responsive_scale * safe_scale),
      .gap = std::max(5.0F * safe_scale, kLogicalPanelGap * ui_scale),
      .inset = std::max(3.0F * safe_scale, kLogicalPanelInset * ui_scale),
      .legend_band =
          std::max(16.0F * safe_scale, kLogicalLegendBandHeight * ui_scale),
      .legend_width =
          std::max(76.0F * safe_scale, kLogicalLegendWidth * ui_scale),
      .legend_height =
          std::max(9.0F * safe_scale, kLogicalLegendHeight * ui_scale),
      .border_width =
          std::max(0.8F * safe_scale, kLogicalBorderWidth * ui_scale),
      .pointer_stroke_width =
          std::max(2.75F * safe_scale, kLogicalPointerStrokeWidth * ui_scale),
      .pointer_outline_width =
          std::max(1.25F * safe_scale, kLogicalPointerOutlineWidth * ui_scale),
      .arrow_head_length =
          std::max(5.5F * safe_scale, kLogicalArrowHeadLength * ui_scale),
      .grid_stroke_width =
          std::max(0.35F * safe_scale, kLogicalGridStrokeWidth * ui_scale),
  };
}

float FitMeshGapToIndexBudget(float width, float height, float desired_gap) {
  float gap = std::max(desired_gap, 0.001F);
  constexpr std::uint64_t vertex_budget = 60000U;
  const auto vertex_count = [width, height](float candidate_gap) {
    const std::uint64_t columns =
        static_cast<std::uint64_t>(std::floor(width / candidate_gap)) + 1U;
    const std::uint64_t rows =
        static_cast<std::uint64_t>(std::floor(height / candidate_gap)) + 1U;
    return columns * rows;
  };
  while (vertex_count(gap) > vertex_budget) {
    gap *= 1.08F;
  }
  return gap;
}
...
std::vector<MeshWarpArtworkShape>
BuildSourceArtwork(int width, int height, std::uint32_t seed,
                   const sk_sp<SkTypeface> &typeface, float device_scale,
                   float responsive_scale) {
  std::vector<MeshWarpArtworkShape> shapes;
  std::mt19937 random(seed);
  const float safe_scale = std::max(device_scale, 0.001F);
  const float compact_mix =
      std::clamp((responsive_scale - 0.58F) / 0.42F, 0.0F, 1.0F);
  const float target_cell_width = std::lerp(104.0F, 160.0F, compact_mix);
  const float target_cell_height = target_cell_width * 0.82F;
  const int columns = std::clamp(
      static_cast<int>(std::lround(static_cast<float>(width) /
                                   (target_cell_width * safe_scale))),
      3, 7);
  const int rows = std::clamp(
      static_cast<int>(std::lround(static_cast<float>(height) /
                                   (target_cell_height * safe_scale))),
      2, 4);
  ...
  for (int row = 0; row < rows; ++row) {
    for (int column = 0; column < columns; ++column) {
      const float jitter_x = (RandomUnit(&random) - 0.5F) * cell_width * 0.34F;
      const float jitter_y = (RandomUnit(&random) - 0.5F) * cell_height * 0.34F;
      const SkPoint center = {
          (static_cast<float>(column) + 0.5F) * cell_width + jitter_x,
          (static_cast<float>(row) + 0.5F) * cell_height + jitter_y};
      const float radius = cell_size * (0.46F + RandomUnit(&random) * 0.15F);
      const float rotation =
          RandomUnit(&random) * 2.0F * std::numbers::pi_v<float>;
      // Four of fourteen slots select HatchLineRect, more than tripling its
      // previous one-in-eleven selection probability.
      const int kind = static_cast<int>(random() % 14U);

      MeshWarpArtworkShape shape;
...
MeshWarpFiddle::MeshWarpFiddle() : MeshWarpFiddle(MeshWarpFiddleOptions{}) {}
MeshWarpFiddle::MeshWarpFiddle(const MeshWarpFiddleOptions &options)
    : options_(options) {}
MeshWarpFiddle::~MeshWarpFiddle() = default;

bool MeshWarpFiddle::EnsureResources() {
  ...
  if (font_manager == nullptr || font_manager->countFamilies() == 0) {
    std::cerr << "[cc-engine/stderr] Mesh warp could not access the shared "
                 "font manager."
              << std::endl;
    return false;
  }
  ...
  if (artwork_typeface_ == nullptr) {
    std::cerr << "[cc-engine/stderr] Mesh warp could not resolve an artwork "
                 "typeface."
              << std::endl;
    return false;
  }
...
void MeshWarpFiddle::Render(double time_seconds) {
  if (!EnsureResources()) {
    return;
  }
  ...
  if (!present.success) {
    std::cerr << "[cc-engine/stderr] Mesh warp could not submit its WebGL "
                 "frame."
              << std::endl;
  }
}

bool MeshWarpFiddle::RebuildScene(int width, int height) {
  device_scale_ =
      static_cast<float>(width / std::max(1.0, static_cast<double>(Width())));
  const MeshLayout layout = MakeMeshLayout(width, height, device_scale_);
  const float padding = layout.padding;
  const float gap = layout.gap;
  const float inset = layout.inset;
  const float legend_band = layout.legend_band;
  ...
  shapes_ = BuildSourceArtwork(image_width, image_height, artwork_seed,
                               artwork_typeface_, device_scale_,
                               layout.responsive_scale);

  graphics::MeshWarperOptions mesh_options;
  // A compact viewport uses a substantially denser logical lattice. Scaling
  // by device_scale keeps this independent of device pixel ratio.
  mesh_options.epsilon_gap = FitMeshGapToIndexBudget(
      source_width_, source_height_,
      std::max(1.75F * device_scale_,
               options_.epsilon_gap * layout.responsive_scale * device_scale_));
...
void MeshWarpFiddle::AdvanceSyntheticDrag(double time_seconds) {
  ...
  active_frame_ = fake_mouse_->AdvanceByRatio(
      std::clamp(options_.pointer_step_ratio, 0.001F, 0.25F));
...
void MeshWarpFiddle::DrawFrame(SkCanvas *canvas, int width, int height) {
  ...
  for (const MeshWarpArtworkShape &shape : shapes_) {
  ...
  if (!reset_pending_) {
    ...
    grid_paint.setStrokeWidth(layout.grid_stroke_width);
  ...
  if (active_frame_.in_drag) {
    ...
    arrow_outline_paint.setStrokeWidth(layout.pointer_stroke_width +
                                       layout.pointer_outline_width);
  ...
  border_paint.setStrokeWidth(layout.border_width);
  canvas->drawRect(top_panel, border_paint);
  canvas->drawRect(bottom_panel, border_paint);

  const float legend_width = layout.legend_width;
  const float legend_height = layout.legend_height;
  ...
  legend_style.border_width =
      std::max(0.65F * device_scale_, layout.responsive_scale * device_scale_);
  legend_style.padding = std::max(
      1.25F * device_scale_, 2.0F * layout.responsive_scale * device_scale_);
  legend_style.corner_radius = 6.0F * layout.responsive_scale * device_scale_;
