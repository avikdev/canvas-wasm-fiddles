...
struct MeshDeformFiddleOptions {
...
class MeshDeformFiddle final : public FiddleBaseWebGL {
public:
  MeshDeformFiddle();
  explicit MeshDeformFiddle(const MeshDeformFiddleOptions &options);
  ~MeshDeformFiddle() override;
...
private:
  ...
  MeshDeformFiddleOptions options_;
  sk_sp<SkTypeface> artwork_typeface_;
  std::unique_ptr<input::FakeMouseActions> fake_mouse_;
  graphics::MeshDeformer deformer_;
  std::vector<MeshDeformArtworkShape> shapes_;
