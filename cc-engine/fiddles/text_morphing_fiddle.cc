...
TextMorphingFiddle::TextMorphingFiddle() {
...
bool TextMorphingFiddle::EnsureResources() {
  ...
  if (font_manager == nullptr || font_manager->countFamilies() == 0) {
    std::cerr << "[cc-engine/stderr] Text morphing could not access the "
  ...
  if (typeface_ == nullptr) {
    std::cerr << "[cc-engine/stderr] Text morphing could not resolve the "
                 "default typeface."
              << std::endl;
    return false;
  }
...
bool TextMorphingFiddle::RebuildGlyphs(float width, float height) {
  ...
  if (!morpher_.Init(source_path_, target_path_, options)) {
    std::cerr << "[cc-engine/stderr] Text morphing initialization failed: "
...
void TextMorphingFiddle::Render(double time_seconds) {
  if (!EnsureResources()) {
    return;
  }
  ...
  if (!present.success) {
    std::cerr << "[cc-engine/stderr] Text morphing could not submit its "
                 "WebGL frame."
              << std::endl;
  }
}

bool TextMorphingFiddle::UpdateState(double time_seconds, int width,
                                     int height) {
...
void TextMorphingFiddle::DrawFrame(SkCanvas *canvas, int width, int height) {
