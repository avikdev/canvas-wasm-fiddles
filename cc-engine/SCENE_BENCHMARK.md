# Scene Benchmark: WebGL versus CPU

The app contains two benchmark entries:

- **Scene Benchmark (WebGL)** (`scene-benchmark-webgl`) uses Ganesh and renders directly into
  WebGL default framebuffer 0.
- **Scene Benchmark (CPU)** (`scene-benchmark-cpu`) uses a raster `SkSurface` backed by an RGBA
  buffer in Wasm memory, then presents it with `putImageData`.

Both call `DrawSceneBenchmark()` in `fiddles/scene_benchmark_builder.cc`. There are no
...
```text
Scene benchmark [backend] last 120 frames (accumulated ms):
